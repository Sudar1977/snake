...
void clearHeadlessTail(snake_t * snake)
{
    ...
    {
        if(snake->tail[i].x == 0 && snake->tail[i].x == 0 ) ///????? ??????????
        {
         //   break;
        }
...
//     init(snake, tail, START_TAIL_SIZE); //?????????????, ????? = 3
    ...
    while( key_pressed != STOP_GAME ) {
        for(int snake_id = MAX_PLAYER_ID+1; snake_id<=last_snake_id; snake_id++) //?????? ??????????? ????????
        {
            if(rand() % 100 < 5)//? ???????????? 5%
            {
                int vkey = rand() % 4;
                if(checkDirection(snakes[snake_id], vkey)) //???????? ???????????? ????? ???????????
                {
                    changeDirection(snakes[snake_id], vkey); // ?????? ??????????? ????????
                }
            }
            ...
            if(frac)
            {
                ...
                snakes[last_snake_id]->direction = (dir+2)%4; // ????? ????? ???? ??????  ? ?????? ???????
            }
        ...
        {
            if(checkDirection(snakes[snake_id], key_pressed)) //???????? ???????????? ????? ???????????
            {
                changeDirection(snakes[snake_id], key_pressed); // ?????? ??????????? ????????
            }
